d3.csv("scripts/gym_data_male.csv").then(function(data) {
  // Converts string values to numbers and calculate sums and counts for averages
  var sumsAndCounts = {};
  data.forEach(function(d) {
    d['1rm_bench'] = +d['1rm_bench'];
    if (sumsAndCounts[d.college]) {
      sumsAndCounts[d.college].sum += d['1rm_bench'];
      sumsAndCounts[d.college].count += 1;
    } else {
      sumsAndCounts[d.college] = { sum: d['1rm_bench'], count: 1 };
    }
  });
  ...
  var svg = d3.select('#male-bench-press');
